...
public class Kalah {
    ...
    /**
     * Get list of legal moves on a given board for a given side.
     *
     * @param board The board to check for moves.
     * @return a list of legal moves.
     */
    public static List<Move> getLegalMoves(Board board, Side side) {
        List<Move> legalMoves = new ArrayList<>();
        for (int i = 1; i <= 7; i++) {
            Move move = new Move(side, i);
            if (isLegalMove(board, move)) legalMoves.add(move);
        }
        return legalMoves;
    }
    ...
    public static Side makeMove(Board board, Move move) {
		...
		*/
        ...
        if (sowHole == 0) {// the store (implies (sowSide == move.getSide()))
            board.setNextPlayerSide(move.getSide());
            return move.getSide();  // move again
        } else {
            board.setNextPlayerSide(move.getSide().opposite());
    ...
    /**
     * Get list of legal moves for a given side.
     *
     * @return a list of legal moves.
     */
    public List<Move> getLegalMoves(Side side) {
        return getLegalMoves(board, side);
    }
